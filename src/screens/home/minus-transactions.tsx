...
const MinusTransactions = memo(() => {
  const { data, size, setSize, isLoading } = useSWRInfinite(
    (index) => `swr.transaction.minus.${index}`,
    async (index) => {
      const page = index.split(".").pop();
      const res = await TransactionApi.minusTransactions({
...
MinusTransactions.displayName = "MinusTransactions";

export { MinusTransactions };
